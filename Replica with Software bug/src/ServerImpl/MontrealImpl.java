...
public class MontrealImpl implements ServerInterface{
	private ORB orb;
	...
	public static Logger log = Logger.getLogger(MontrealImpl.class.getName());
...
//		mainMap.put("mon0001", new HashMap<String, Integer>());
//		mainMap.put("mon0002", new HashMap<String, Integer>());
//		mainMap.put("mon0003", new HashMap<String, Integer>());
//		mainMap.get("mon0001").put("java", 1);
//		//semaphoreCheck.put("mon0001",true);
//		mainMap.get("mon0002").put("ds", 1);
//		//semaphoreCheck.put("mon0002",true);
//		mainMap.get("mon0003").put("network", 1);
//		//semaphoreCheck.put("mon0003",true);
//	}
//	}
	...
	public String borrowItem(String userID, String itemID) {
		...
		if ((!userID.substring(0, 3).toUpperCase().equals("MON")) && !borrowedItemMap.isEmpty()
			...
			for(String bookIdString : arrayList) {
				if(bookIdString != null && "MON".contains(bookIdString.substring(0, 3).toUpperCase()))
		...
		int qty = 0;
		if (itemID.substring(0, 3).toUpperCase().equals("MON") && mainMap.containsKey(itemID)) {
			//while(!semaphoreCheck.get(itemID));
			//semaphoreCheck.put(itemID,false);
			...
			if (qty == 0 && itemID.substring(0, 3).toUpperCase().equals("MON") && mainMap.containsKey(itemID)) {
		...
		} else if (!itemID.substring(0, 3).trim().toUpperCase().equals("MON".trim())) {
...
//					for(String borrowItemIdString : string) {
	...
	public String queueImplementation(String itemId, String userId) {
		...
		try {
			...
			if (itemId.substring(0, 3).equals("MCG")) {
				...
				return objImplSupport.ipcConnection(userId, itemId, "4", itemName, ConstantVal.MCGILL_SERVER, oldItemId)
						.trim();
	...
	public String findItemOtherServers(String userID, String itemName) {
		...
		try {
			...
			responseString += objImplSupport
					.ipcConnection(userID, itemID, methodsString, itemName, ConstantVal.MCGILL_SERVER, oldItemId)
					.trim();
	...
	public String oldItemReturn(String studentID, String oldItemID, String newItemID) {
		...
		String userId = null;
		if (oldItemID.substring(0, 3).equalsIgnoreCase("MON")) {
			if (borrowedItemMap.get(studentID).contains(oldItemID.trim())) {
				...
				|| oldItemID.substring(0, 3).equalsIgnoreCase("MCG")) {
			ImplSupport objImplSupport = new ImplSupport();
			try {
				response = objImplSupport.ipcConnection(studentID, newItemID, "6", itemName,
						(oldItemID.substring(0, 3).trim().toUpperCase().equals("CON") ? ConstantVal.CON_SERVER
								: ConstantVal.MCGILL_SERVER),
						oldItemID).trim();
				return response;
			} catch (SocketException e) {
	...
	public String newItemBorrow(String studentID, String oldItemID, String newItemID) {
		...
		Integer qty = null;
		if ((!studentID.substring(0, 3).toUpperCase().equals("MON")) && !borrowedItemMap.isEmpty() //newly added line
		...
		if (newItemID.substring(0, 3).equalsIgnoreCase("MON")) {
			if (mainMap.containsKey(newItemID)) {
				...
				|| newItemID.substring(0, 3).equalsIgnoreCase("MCG")) {
			ImplSupport objImplSupport = new ImplSupport();
			try {
				response = objImplSupport.ipcConnection(studentID, newItemID, "7", itemName,
						(newItemID.substring(0, 3).trim().toUpperCase().equals("CON") ? ConstantVal.CON_SERVER
								: ConstantVal.MCGILL_SERVER),
						oldItemID).trim();
			} catch (SocketException e) {
				// TODO Auto-generated catch block
	...
	public String exchangeItem(String studentID, String newItemID, String oldItemID) {
		...
		MontrealImpl montrealImpl = new MontrealImpl();
		initLogger(log, studentID);
		newItemId = montrealImpl.newItemBorrow(studentID, oldItemID, newItemID);
		checkoldItemId = montrealImpl.oldItemReturn(studentID, oldItemID, newItemID);
		...
		log.info("Not exchanged");
		return "Item can't exchange";

	}

	public void parseBookInfo(List<BookData> bookDatasList) {
		System.out.println("parse");
		for (BookData bookData : bookDatasList) {
			if (innerMap==null) {
