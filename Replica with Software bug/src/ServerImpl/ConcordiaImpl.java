...
public class ConcordiaImpl implements ServerInterface {
	private ORB orb;
	...
	public static Logger log = Logger.getLogger(ConcordiaImpl.class.getName());
//	{
//		if (mainMap==null) {
//		//System.out.println(queueHashMap);
//		mainMap = new HashMap<String, HashMap<String, Integer>>();
//		mainMap.put("con0001", new HashMap<String, Integer>());
//		mainMap.put("con0002", new HashMap<String, Integer>());
//		mainMap.put("con0003", new HashMap<String, Integer>());
//		mainMap.get("con0001").put("java", 1);
//		//semaphoreCheck.put("con0001",true);
//		mainMap.get("con0002").put("ds", 1);
//		//semaphoreCheck.put("con0002",true);
//		mainMap.get("con0003").put("network", 1);
//		//semaphoreCheck.put("con0003",true);
//	}
//	}
	...
	public String removeItem(String managerID, String itemID, int quantity) {
		...
		if (quantity > 0 && innerMap.size() > 0 && quantity <= quantityExisted) {
			...
			log.info("quantity greater than existing quantity");
			//semaphoreCheck.put(itemID,true);
	...
	public String borrowItem(String userID, String itemID) {
		...
		if ((!userID.substring(0, 3).toUpperCase().equals("CON")) && !borrowedItemMap.isEmpty()
		...
		int qty = 0;
		if (itemID.substring(0, 3).toUpperCase().equals("CON") && mainMap.containsKey(itemID)) {
			//while(!semaphoreCheck.get(itemID));
			//semaphoreCheck.put(itemID,false);
			...
			if (qty == 0 && itemID.substring(0, 3).toUpperCase().equals("CON") && mainMap.containsKey(itemID)) {
		...
		} else if (!itemID.substring(0, 3).trim().toUpperCase().equals("CON")) {
	...
	public String returnItem(String userID, String itemID) {
		...
		initLogger(log, userID);
		try {
			if (itemID.substring(0, 3).toUpperCase().equals("CON")) {
	...
	public String queueImplementation(String itemId, String userId) {
		...
		try {
			...
			} else if (itemId.substring(0, 3).toUpperCase().equals("MCG")) {
				ImplSupport objImplSupport = new ImplSupport();
	...
	public String newItemBorrow(String studentID, String oldItemID, String newItemID) {
		...
		Integer qty = null;
		if ((!studentID.substring(0, 3).toUpperCase().equals("CON")) && !borrowedItemMap.isEmpty() //newly added line
		...
		if (newItemID.substring(0, 3).equalsIgnoreCase("CON".trim())) {
	...
	public String exchangeItem(String studentID, String newItemID, String oldItemID) {
		...
		ConcordiaImpl concordiaImpl = new ConcordiaImpl();
		initLogger(log, studentID);

		newItemId = concordiaImpl.newItemBorrow(studentID, oldItemID, newItemID);
		checkoldItemId = concordiaImpl.oldItemReturn(studentID, oldItemID, newItemID);
